...
function createTask(bg, data, title, check) {
  ...
  newTaskBox.classList = "task fade shadow-out";
  newTaskBox.classList.add(bg);
  newTaskBox.dataset.division = data;
  taskTitle.classList = "task-title mb-0";
  ...
  // drag
  newTaskBox.draggable = "true";
  newTaskBox.ondragstart = function (e) {
    e.dataTransfer.setData("taskTxt", e.target.firstChild.innerHTML);
  };
...
//
//
// tasks append | appending tasks :
function appendTask(item) {
  taskContainer.append(item);
  if (item.dataset.division != currentDiv) {
    item.classList.add("fade");
    setTimeout(function () {
      item.classList.add("d-none");
    }, 300);
...
// tasks append | appending tasks End !
//
//
// theme change | dark & light background :
function winTheme() {
